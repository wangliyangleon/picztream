...
- **选片交互**（`pzt open` 内）：`h`/`l` 上一张/下一张的秒切浏览，带上限的标签分组，`f` 按标签筛选（`f` + 数字进入该标签的筛选视图、`f f` 清除；早期是 `g`，已改成 Filter 首字母）、控制台 `/filter` 二级筛选，废片系统标签，recipe 风格切换与同步预览，导出。控制台 `/dedup <范围> [--ai]` 触发近似重复检测（范围本身是系统标签时拒绝，见上面"批量作用域与排除规则"），带 `--ai` 时簇内改走 AI 锦标赛选保留项（开跑前先报出精确的组数与比较次数等一次确认，取消则零写入；跑起来之后逐次比较报进度）。整条命令阻塞期间可以按 Ctrl-C 取消这一次去重（不退出 `pzt open`）：按下立刻回显、在下一个检查点生效、零写入。不在这段时间里的 Ctrl-C 仍然是干净退出 `pzt`——终端由 `cli/term/signal_restore.h` 在信号路径上还原，默认信号处置不做栈回退、RAII 析构跑不到。控制台 `/recipe <范围>`（T-15 票 C）对范围内全部图片套用同一个配方或批量清除：作用域写在命令里、配方回车之后在菜单上选（那个菜单是 `r` 菜单的子集，只有 `1`-`9` 与 `0`/`r` 清除，`v`/`c`/`d` 不出现），执行前**总是**弹两行确认并报出「范围内 N 张、其中 M 张已有配方会被覆盖且无法还原」，按 `n`/Esc 零写入；一个事务包住 N 次 `UPDATE`，不做进度也不做中途取消。
- **色彩配方**：`recipe`（`list`/`rename`/`delete` 及交互式创建流程）。
- **导出**：`export`（按标签导出到文件夹，或 `--all-keep` 导出全部并默认排除废片/重复）。
- **多语言**：默认中文，`config.json` 的 `lang` 字段可切英文（缺省时回退系统 `LANG`）。i18n 只在 cli 层，`core` 不承载任何面向用户的文案。**这条约束管的是"文案"，不是"标识符"**：系统标签在库里永远存 canonical 中文名（`废片`/`重复`），它的**显示名**跟着界面语言走、归 `cli/i18n`；而用户在范围里打 `#Reject`/`#reject` 时要解析成哪个标签，是标识符解析、不随界面语言变，归 `core/scope`（T-16 决策 D-3，见 `core/scope/scope.h`）。两者容易混成一件事，实际是同一个词的两种身份。这套别名有第二个消费者：`pzt images --json` 的 `system_tags` 字段（T-25），agent 按它判断"这张是不是废片/重复"，而不是拿中文存储名去比对显示用的 `tags` - 后者在 core 改名或系统标签名 i18n 化时会**静默地不过滤任何东西**。别名的唯一定义在 `core/tagging`，`core/scope` 与 headless 输出共用同一组常量。

- **批量作用域与排除规则**：`*` / `#标签名` / `#"带空格的标签名"` / `.` 这套范围写法，以及"批量操作默认排除废片/重复"这条 F-26 策略，唯一实现都在 `core/scope`，交互与 headless 两个入口共用（T-16）。`.` 是 T-15 加的第四支，语义是**调用方给定的显式 id 集合**（交互侧传当前视图，headless 无从可传、报独立的 `no_current_view` 而不是语法错）；它的 `scope_tag` 恒为空，因此接不上 F-26 的对称例外。交互侧三条带作用域的命令（`/dedup`、`/ai_eval`、`/recipe`）都经 `browse.cpp` 里同一处 `resolve_scope_with_view` 把当前视图交给 core，全 cli 只有那一处传视图。各命令排哪些标签、受哪个开关控制仍由调用方决定 - 统一的是机制不是策略。四处刻意的不对称：`/dedup` 拒绝把系统标签当范围（`#废片` 全被排空必然无结果，报错比静默"0 组"诚实），而 `/ai_eval` 与 `export` 允许并走 F-26 的对称例外；dedup 的排废片**无条件、无开关**（`settings` 里没有对应旋钮），而 eval/export 各有 `eval_reject`/`export_reject`/`export_dup`；第四处是 `/recipe`（T-15 票 C）**完全不排除**任何东西、也不新增开关 - F-26 那三个实例各有具体失效模式（eval 把钱花在已判死的照片上、export 把垃圾交出去、dedup 让废片当上 keeper），套配方一个都没有，全部代价是一次 `UPDATE`，而"套了会不会流出去"已由 `export` 自己的排除挡住；何况 `.` 的 `scope_tag` 恒为空、接不上对称例外，若默认排除则 `/filter reject` → `/recipe .` 会被排空成静默 no-op，正是 T-16 刚修掉的那个失效模式的复刻。

### 3.2 Headless 命令面（面向 agent，`--json`）

供 agent 层通过子进程驱动的原子命令，每个都输出结构化 JSON。`--scope` 的写法与交互侧完全一致（同一份 `core/scope` 解析，因此也认 `#Reject`/`#Duplicate` 这类系统标签别名；T-16 之前 headless 这一份是独立重写的，不认别名）：`new --json`（导入）、`images`（读项目当前状态）、`dedup`（近似重复检测，范围是系统标签时报 `system_tag_scope`）、`curate`（选片：关 AI 时是多样性选片，开 AI 时先多样性预筛出预选集、评估、再由模型按选片简述连选带排并产出文案）、`recipe suggest`（看图选风格）、`recipe apply`（应用风格，`set_image_recipe` 的薄壳）、`tag apply`/`tag clear`（打标签/清标签）、`export-images`（导出选中图为字节）。
