...
- **选片交互**（`pzt open` 内）：`h`/`l` 上一张/下一张的秒切浏览，带上限的标签分组，`f` 按标签筛选（`f` + 数字进入该标签的筛选视图、`f f` 清除；早期是 `g`，已改成 Filter 首字母）、控制台 `/filter` 二级筛选，废片系统标签，recipe 风格切换与同步预览，导出。控制台 `/dedup <范围> [--ai]` 触发近似重复检测（范围本身是系统标签时拒绝，见上面"批量作用域与排除规则"），带 `--ai` 时簇内改走 AI 锦标赛选保留项（开跑前先报出精确的组数与比较次数等一次确认，取消则零写入；跑起来之后逐次比较报进度）。整条命令阻塞期间可以按 Ctrl-C 取消这一次去重（不退出 `pzt open`）：按下立刻回显、在下一个检查点生效、零写入。不在这段时间里的 Ctrl-C 仍然是干净退出 `pzt`——终端由 `cli/term/signal_restore.h` 在信号路径上还原，默认信号处置不做栈回退、RAII 析构跑不到。
...
- **批量作用域与排除规则**：`*` / `#标签名` / `#"带空格的标签名"` / `.` 这套范围写法，以及"批量操作默认排除废片/重复"这条 F-26 策略，唯一实现都在 `core/scope`，交互与 headless 两个入口共用（T-16）。`.` 是 T-15 加的第四支，语义是**调用方给定的显式 id 集合**（交互侧传当前视图，headless 无从可传、报独立的 `no_current_view` 而不是语法错）；它的 `scope_tag` 恒为空，因此接不上 F-26 的对称例外。各命令排哪些标签、受哪个开关控制仍由调用方决定 - 统一的是机制不是策略。三处刻意的不对称：`/dedup` 拒绝把系统标签当范围（`#废片` 全被排空必然无结果，报错比静默"0 组"诚实），而 `/ai_eval` 与 `export` 允许并走 F-26 的对称例外；dedup 的排废片**无条件、无开关**（`settings` 里没有对应旋钮），而 eval/export 各有 `eval_reject`/`export_reject`/`export_dup`。
