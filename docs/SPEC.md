...
- **多语言**：默认中文，`config.json` 的 `lang` 字段可切英文（缺省时回退系统 `LANG`）。i18n 只在 cli 层，`core` 不承载任何面向用户的文案。**这条约束管的是"文案"，不是"标识符"**：系统标签在库里永远存 canonical 中文名（`废片`/`重复`），它的**显示名**跟着界面语言走、归 `cli/i18n`；而用户在范围里打 `#Reject`/`#reject` 时要解析成哪个标签，是标识符解析、不随界面语言变，归 `core/scope`（T-16 决策 D-3，见 `core/scope/scope.h`）。两者容易混成一件事，实际是同一个词的两种身份。
