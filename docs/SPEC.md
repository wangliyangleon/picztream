...
**stderr 是带外通道**（T-8 起）：承载失败时的错误对象，以及长命令运行期间的进度（一行一个 `{"progress":{...}}` 对象）。它不属于结果，调用方可以完全忽略而不影响正确性 - `pzt dedup --ai` 一次内部要跑 Σ(簇大小-1) 次视觉推理、量级到分钟，原子调用本身没有观测面，这条带外通道是补上观测而不动 stdout 原子性的办法。读取方必须把解析不了的行当正常情况忽略（stderr 上还混着调参用的纯文本明细），并且**无条件排水**：只读一条管道会让另一条填满后把子进程卡死。
