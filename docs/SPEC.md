...
- **选片交互**（`pzt open` 内）：`h`/`l` 上一张/下一张的秒切浏览，带上限的标签分组，`f` 按标签筛选（`f` + 数字进入该标签的筛选视图、`f f` 清除；早期是 `g`，已改成 Filter 首字母）、控制台 `/filter` 二级筛选，废片系统标签，recipe 风格切换与同步预览，导出。控制台 `/dedup <范围> [--ai]` 触发近似重复检测，带 `--ai` 时簇内改走 AI 锦标赛选保留项（开跑前先报出精确的组数与比较次数等一次确认，取消则零写入）。
