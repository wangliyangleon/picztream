...
`~/.config/pzt/config.json` 跟库不一样，**没有版本号也不需要**：解析是逐字段 `assign_if_present`，认识的字段读走、不认识的原样忽略、缺的用默认值。因此删掉一个配置项不会让老用户的配置文件报错，加一个也不会让老版本崩。代价是**拼错的字段名不会有任何提示**，只会静默用默认值。

已删除的字段（留在文件里无害，但不再有任何效果）：

| 字段 | 删除时间 | 说明 |
|---|---|---|
| `dedup_reject` | 2026-08-13（T-16 / [#27](https://github.com/wangliyangleon/picztream/issues/27)） | 语义曾是"去重时不排除废片"。自 2026-07-21 起就已经完全无效 - core 侧无条件排废片，这个开关只让 cli 层跳过自己那份重复的排除。要让它真正生效需要先把簇内 keep 选择改成废片感知，否则一张废片会因为拍摄时间最新而当上 keeper、把好邻居全打成"重复"。 |
