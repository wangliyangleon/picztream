...
5. **预览接入 `pzt open` 现有渲染路径**：在 `resize_rgba` 降采样之后、发给终端之前插入 `recipe::render(..., thread_count=1)` 这一步。这一步还没有 `r` 键，先用 increment 3 的调试命令给某张图片手动设置一个 recipe，再 `pzt open` 该项目肉眼确认预览正确应用了风格、`key-to-render` 延迟汇总仍在正常范围（对照 Phase 0 spike 的预期量级）
6. **`r` 前缀键完整交互**（对应 M1_PRD.md"应用与预览"一节），内部再拆子步骤：
   - **6.1 应用/清除**：`r` + 预设数字 + version 数字应用、`r` + `0` 应用 Origin、`r` + `r` 快捷清除，复用 increment 3/5 已经打通的 `set_image_recipe`/渲染路径，只是把入口从调试命令换成真实按键，风格上照抄 `handle_g_key_prompt` 两层菜单的读键模式
   - **6.2 创建**：`r` + `c`——选一个基础预设、用 `read_text_line`（`space c` 已经验证过的文本输入组件）依次读高光/暗光/白平衡红/蓝几个数值、可选的名字，调 `create_version`，对齐 `handle_create_tag_flow` 的多步骤读取风格
   - **6.3 删除**：`r` + `d` + 预设数字 + version 数字，两层选择，预设不出现在删除列表里，调 `delete_version`（软删除），对齐 `handle_delete_tag_submenu` 的确认交互
   - **6.4 收尾**：退休这个里程碑过程中新增的临时调试命令（`pzt recipe create-debug`/`apply-debug`/`clear-debug`/`pzt color-debug`），只保留 M1_PRD.md 定的正式命令（`pzt recipe list`/`rename`/`delete`），照抄 M0 increment 6.4.7 的收尾模式
