...
```cpp
auto input = read_text_line_with_placeholder(...);
if (!input) return "";  // Esc,静默取消
if (!input->empty() && (*input)[0] == '/') {
  return handle_ai_console_command(evaluation_worker, project_id, input->substr(1));
}
// 原来的单图评估流程,不变
```

`handle_ai_console_command` 拆出第一个词当命令名，`ai_eval` 再拆第二个词当 scope、剩下的整段（去掉中间那一个分隔空格）当额外指引；`tasks` 不需要再解析参数：
...
std::string handle_ai_console_command(pzt::core::EvaluationWorker& evaluation_worker,
  ...
  if (command == "ai_eval") {
    ...
    if (!extra_guidance.empty() && extra_guidance.front() == ' ') extra_guidance.erase(0, 1);
    return handle_ai_eval_command(evaluation_worker, project_id, scope, extra_guidance);
  }
  return pzt::cli::i18n::msg_ai_unknown_command();
}
```

`handle_ai_eval_command`：scope 是 `*` 就 `list_images(project_id)`，否则 `find_tag_by_name` 找标签 id 再 `filter_by_tag`（跟 `g` 筛选复用的是同一个函数，见 `core/browse/browse.h`）——标签不存在时提示错误，不静默当成空范围处理。拿到图片列表之后逐张查 `get_image(id)->evaluation`，已经有值的跳过，没有的调 `evaluation_worker.request(...)`，最后汇总提交了多少张：
...
std::string handle_ai_eval_command(pzt::core::EvaluationWorker& evaluation_worker,
                                    ...
                                    const std::string& extra_guidance) {
  std::vector<pzt::core::ImageRef> images;
  if (scope == "*") {
    images = pzt::core::list_images(project_id);
  } else {
    auto tag_id = pzt::core::find_tag_by_name(project_id, scope);
    if (!tag_id) return pzt::cli::i18n::msg_ai_eval_tag_not_found(scope);
    auto filtered = pzt::core::filter_by_tag(*tag_id);
    if (!filtered.ok()) return pzt::cli::i18n::err_filter_failed();  // 复用现成的错误文案
    images = filtered.value();
  }

  int submitted = 0;
  for (const auto& ref : images) {
    auto info = pzt::core::get_image(ref.id);
    if (info && info->evaluation) continue;  // 已经评估过,跳过
    if (evaluation_worker.request(ref.id, pzt::core::Provider::Gemini, extra_guidance)) {
      ++submitted;
    }
  }
...
新增几条：`msg_ai_unknown_command()`（`/` 后面不匹配任何已知命令）、`msg_ai_eval_tag_not_found(tag_name)`、`msg_ai_eval_submitted(count)`（批量提交了几张，`count` 为 0 时文案要能自然表达"没有需要处理的"，不是"提交了 0 张"这种生硬的说法）、`msg_ai_tasks_status(queued, processing)`（`/tasks` 的状态文案，`processing` 决定要不要多一句"有一个正在处理中"）。
...
这次新增（`/ai_eval`、`/tasks` 批量命令）：

6. **`EvaluationWorker::queue_status()`**：新增聚合状态查询，单元测试覆盖"空队列"、"有排队没在处理"、"有排队且有一个在处理"三种状态下 `queued`/`processing` 的值是否正确
7. **`cli/commands/browse.cpp`**：`handle_ai_prompt_flow` 加 `project_id` 参数、`/` 分支解析（`handle_ai_console_command`/`handle_ai_eval_command`/`handle_tasks_command`）、新增 i18n 字符串——沿用"cbreak 按键循环没法自动化测试"的一贯限制，`handle_ai_eval_command` 内部调用的 `list_images`/`find_tag_by_name`/`filter_by_tag`/`get_image` 都是已经测过的现成函数，这次不需要为编排逻辑本身另开单元测试，靠真机验证命令解析和批量提交的行为
8. **真机验收**：`/ai_eval *`、`/ai_eval <标签名>`、`/tasks` 在真实项目上跑一遍，确认跳过已评估图片的行为正确、`/tasks` 的数字随处理进度正确变化、未知命令有清楚提示
...
* **`/ai_eval <标签名>` 的解析边界**：假设标签名不含空格，见 `docs/M3_PRD.md` 同一条风险的说明
