...
* **测量触发**: F-37(编译时长)、F-38(每帧连接开销)——先出数字再决定动不动。
* **待产品拍板**: F-28(有序标签调序二选一)、F-41(pzt list 计数 vs 改 PRD 措辞)、F-44(筛选视图批量打标签,等 `/filter` 用出手感)。
...
**Wave 1 收口结论**:F-39/F-40、F-35/F-42、F-34/F-36 三个提交完成,P3 里"无需触发的低风险项"清空。P3 剩余项按性质分三类留待后续:**测量触发**(F-37 编译时长、F-38 每帧连接开销,需先出数字再决定动不动)、**待产品拍板**(F-28 有序标签调序、F-41 pzt list 计数二选一、F-44 筛选视图批量打标签,等真实使用手感或一句话拍板)、**留档观察不动**(F-45/F-46/F-48,以及 F-36 残留的计时子项)。其余交互打磨(F-27/F-29/F-30/F-43/F-47)维持"用出需要再做"。
