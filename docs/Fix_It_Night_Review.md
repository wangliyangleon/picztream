...
方案分支:
* (a) 推荐: 数字 `9` 固定分配给"重复"(呼应此前讨论过但搁置的"1-8 动态 + 9 保留"方向),`tags_for_menu` 截断从 9 改 8,space/space-/g 三处菜单统一显示 `9:重复`(可以仅当项目里已存在该标签时显示,避免没跑过 dedup 的项目多一个死项)
* (b) 系统标签追加在用户标签之后动态编号(实现最省,但编号会漂移,违背"数字键肌肉记忆固定"的既有设计原则)
* (c) 控制台加 `/filter #标签名`(能救急,但和 g 菜单形成两套筛选入口,交互不统一)
...
方案: (1) 参数从 `/dedup` 可选参数或配置(F-12)传入;(2) `DedupSummary` 加 `skipped_no_capture_time` 计数,结果文案带出("另有 N 张缺拍摄时间未参与");(3) --debug 模式下把每个候选簇的两两汉明距离打进 stderr(debug 面板可见),给调参提供事实依据。
...
方案分支(可组合,建议先做 a):
* (a) 虚拟筛选: g 菜单(或控制台 `/filter fail` / `/filter unevaluated`)增加"不达标/未评估"两个非标签筛选项,复用现有筛选视图机制(images 换池 + banner 标注)
* (b) 排序: `/sort score` 把当前池按综合分升序重排(浏览顺序本来就是内存里的 vector,改动小)
* (c) 批量动作: `/reject_failed` 把范围内不达标的全部打废片(激进,须带确认;建议放最后,等 a/b 用出手感)
...
方案: `~/.config/pzt/config.json`,cli 层新增 `cli/config` 读取(nlohmann 已在依赖树);core 不感知配置,值经既有函数参数传入。第一批键建议: `ai.provider`、`dedup.time_window_seconds`、`dedup.hash_threshold`、`prefetch.window`、`ui.width_ratio`、`lang`。键位自定义放第二批(涉及按键分发表重构,工作量另算)。
...
**F-26 批量范围默认排除废片(/ai_eval * 与 /dedup *)**
类别: 需求proposal + 交互打磨 | 来源视角: 产品 + 用户

背景: 见 2.1 场景 D。已废弃的照片被评估是白花钱,被 dedup 拉进组里甚至可能当 keep_id。

方案分支: (a) `*` 默认排除带废片标签的图,文案注明"已跳过废片 N 张",另设 `*all` 显式全量;(b) 保持现状只加文案提示。推荐 (a),行为变化写进 PRD。

难度 S | 复杂度 低(有语义变化需拍板) | 优先级 P2
...
| 编号 | 一句话 | 类别 | 来源视角 | 难度 | 复杂度 | 优先级 |
|---|---|---|---|---|---|---|
| F-01 | "重复"标签菜单不可达,dedup 复核链断裂 | Bug | 产品+用户+工程 | S-M | 低 | **P0** |
| F-02 | AI 评估上传全分辨率原图 | Bug+性能 | 工程+用户 | S | 低 | P1 |
| F-03 | AI 失败静默,用户无从得知 | Bug+交互 | 用户+工程 | S | 低 | P1 |
| F-04 | 无 busy_timeout,并发写可 terminate | 健壮性 | 工程+架构 | S | 低 | P1 |
| F-05 | 无异常边界,崩溃留坏终端 | 健壮性 | 工程+架构 | S | 低 | P1 |
| F-06 | 扫描目录异常直接崩溃 | 健壮性+Bug | 工程+用户 | S | 低 | P1 |
| F-07 | 批量命令 N+1 连接,含 dedup 冗余统计 | 性能+纯清理 | 工程+架构 | S-M | 低 | P1 |
| F-08 | dedup 参数写死/无时间图静默跳过/无观测 | Bug+需求 | 用户+产品+工程 | M | 中 | P1 |
| F-09 | 评估结果不可行动(筛选/排序/批量) | 需求proposal | 产品+用户 | M | 中 | P1 |
| F-10 | 供应商硬编码,Claude 不可达 | 需求+纯清理 | 产品+工程 | S | 低 | P1 |
| F-11 | dedup 后无复核入口动线 | 交互打磨 | 产品+用户 | S | 低 | P1 |
| F-12 | 配置文件机制缺失 | 需求proposal | 产品+架构 | M | 中 | P1 |
| F-13 | 有界解码(内存/60MP/dedup 提速) | 性能/内存 | 工程+架构+产品 | M | 中 | P2 |
| F-14 | prefetch get 持锁拷贝整图 | 性能/内存 | 工程 | S | 低 | P2 |
| F-15 | set_current 全项目路径表 O(n)/键 | 性能+纯清理 | 工程 | S | 低 | P2 |
| F-16 | 抽 core/media,消依赖倒挂与三份拷贝 | 纯重构 | 架构 | S-M | 低 | P2 |
| F-17 | 写路径 sqlite3_step 不检查 | 工程规范+Bug | 工程 | S | 低 | P2 |
| F-18 | dedup 忽略 add_tag 结果 | Bug(edge) | 工程 | S | 低 | P2 |
| F-19 | Result 加 [[nodiscard]]/NDEBUG 防线 | 工程规范 | 工程 | S | 低 | P2 |
| F-20 | resize 残影,无 SIGWINCH 处理 | Bug+交互 | 用户+工程 | S-M | 低 | P2 |
| F-21 | curl NOSIGNAL/连接超时 | 工程规范 | 工程 | S | 低 | P2 |
| F-22 | 控制台纯函数抽库补测 | 纯重构+规范 | 架构+工程 | S | 低 | P2 |
| F-23 | AGENTS.md 状态过期 | 文档修正 | 架构 | S | 低 | P2 |
| F-24 | 会话续点 | 需求proposal | 产品+用户 | S | 低 | P2 |
| F-25 | 阻塞后清 tty 输入缓冲 | Bug+交互 | 用户+工程 | S | 低 | P2 |
| F-26 | 批量范围默认排除废片 | 需求+交互 | 产品+用户 | S | 低 | P2 |
| F-27 | 未标记剩余计数 | 交互打磨 | 用户 | S | 低 | P3 |
| F-28 | 有序标签调序缺口(先拍板) | 需求+观察 | 产品 | M | 中 | P3 |
| F-29 | unarchive | 需求proposal | 用户 | S | 低 | P3 |
| F-30 | version 应用内改名缺口 | 交互+观察 | 用户 | S | 低 | P3 |
| F-31 | 删门面 find_image_by_path 死代码 | 纯清理 | 工程 | S | 低 | P3 |
| F-32 | 预设播种每调用重算 LUT | 纯清理+性能 | 工程 | S | 低 | P3 |
| F-33 | 迁移残留 ensure_column_dropped x4 | 纯清理 | 工程 | S | 低 | P3 |
| F-34 | raw alpha 字节 0 改 255 | 工程规范 | 工程 | S | 低 | P3 |
| F-35 | 宽度表补 emoji | Bug(edge) | 工程 | S | 低 | P3 |
| F-36 | 规范杂项包(短写/usleep/计时/p95等) | 工程规范 | 工程 | S | 低 | P3 |
| F-37 | api.h 头部重量 | 纯重构+观察 | 架构 | M | 低 | P3 |
| F-38 | 门面连接开销重测(维持观察) | 观察+性能 | 架构 | - | 低 | P3 |
| F-39 | dedup 分组顺序确定化 | 纯清理 | 工程 | S | 低 | P3 |
| F-40 | IN 子句分块防变量上限 | 健壮性(edge) | 工程 | S | 低 | P3 |
| F-41 | pzt list 标签计数 drift 收口 | 文档修正 | 产品 | S | 低 | P3 |
| F-42 | read_text_line 超宽换行 | Bug(edge) | 工程 | S | 低 | P3 |
| F-43 | 导出路径记忆 | 交互打磨 | 用户 | S | 低 | P3 |
| F-44 | 筛选视图批量打标签 | 需求proposal | 用户 | M | 中 | P3 |
| F-45 | Esc 20ms 阈值远程场景 | 观察记录 | 工程 | - | - | P3 |
| F-46 | tmpfile 残留 / t=s 验证 | 观察记录 | 工程 | - | - | P3 |
| F-47 | 纯 JPEG 导入进度 | 交互打磨 | 用户 | S | 低 | P3 |
| F-48 | 低频功能盘点(维持现状) | 观察记录 | 用户 | - | - | P3 |
