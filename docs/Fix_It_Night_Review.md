...
方案: 抽 `cli/console`(依赖 cli/text/i18n)或先并入 cli/text;补 doctest 用例(引号闭合/未闭合、多空格、`#` 边界、CJK 宽度换行)。顺带记录: browse.cpp 已约 1514 行(fix-it 的 `/filter`、`/help`、`e` 导出二级菜单等落地后从 1138 增长),下一个大特性进来前先抽帧渲染函数,本条不含该项。
...
类别: 健壮性(edge) | 来源视角: 工程
3 万张以上项目才会触发。F-07 落地的 `images_with_tag` 已带 500 分块,但那是 tagging 层的独立 helper;dedup 自己的 `load_metas` IN 子句(dedup.cpp:71-81)仍逐个绑定变量、未分块,本条指的是这一处,仍未收口。
难度 S | 复杂度 低 | 优先级 P3
...
**P2 正常排期**: F-22 -> F-20 -> F-24 -> F-14/F-15。(F-16 已完成,见文末"已完成(2026-07 P2 收尾批次)")
