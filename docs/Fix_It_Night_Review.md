...
|---|---|---|---|---|---|---|---|
| F-01 | "重复"标签菜单不可达,dedup 复核链断裂 | Bug | 产品+用户+工程 | S-M | 低 | **P0** | 未实现 |
| F-02 | AI 评估上传全分辨率原图 | Bug+性能 | 工程+用户 | S | 低 | P1 | 未实现 |
| F-03 | AI 失败静默,用户无从得知 | Bug+交互 | 用户+工程 | S | 低 | P1 | 未实现 |
| F-04 | 无 busy_timeout,并发写可 terminate | 健壮性 | 工程+架构 | S | 低 | P1 | 未实现 |
| F-05 | 无异常边界,崩溃留坏终端 | 健壮性 | 工程+架构 | S | 低 | P1 | 未实现 |
| F-06 | 扫描目录异常直接崩溃 | 健壮性+Bug | 工程+用户 | S | 低 | P1 | 未实现 |
| F-07 | 批量命令 N+1 连接,含 dedup 冗余统计 | 性能+纯清理 | 工程+架构 | S-M | 低 | P1 | 未实现 |
| F-08 | dedup 参数写死/无时间图静默跳过/无观测 | Bug+需求 | 用户+产品+工程 | M | 中 | P1 | 未实现 |
| F-09 | 评估结果不可行动(筛选/排序/批量) | 需求proposal | 产品+用户 | M | 中 | P1 | 未实现 |
| F-10 | 供应商硬编码,Claude 不可达 | 需求+纯清理 | 产品+工程 | S | 低 | P1 | 未实现 |
| F-11 | dedup 后无复核入口动线 | 交互打磨 | 产品+用户 | S | 低 | P1 | 未实现 |
| F-12 | 配置文件机制缺失 | 需求proposal | 产品+架构 | M | 中 | P1 | 未实现 |
| F-13 | 有界解码(内存/60MP/dedup 提速) | 性能/内存 | 工程+架构+产品 | M | 中 | P2 | 未实现 |
...
| F-17 | 写路径 sqlite3_step 不检查 | 工程规范+Bug | 工程 | S | 低 | P2 | 未实现 |
| F-18 | dedup 忽略 add_tag 结果 | Bug(edge) | 工程 | S | 低 | P2 | 未实现 |
| F-19 | Result 加 [[nodiscard]]/NDEBUG 防线 | 工程规范 | 工程 | S | 低 | P2 | 未实现 |
| F-20 | resize 残影,无 SIGWINCH 处理 | Bug+交互 | 用户+工程 | S-M | 低 | P2 | 未实现 |
| F-21 | curl NOSIGNAL/连接超时 | 工程规范 | 工程 | S | 低 | P2 | 未实现 |
| F-22 | 控制台纯函数抽库补测 | 纯重构+规范 | 架构+工程 | S | 低 | P2 | 未实现 |
| F-23 | AGENTS.md 状态过期 | 文档修正 | 架构 | S | 低 | P2 | 未实现 |
| F-24 | 会话续点 | 需求proposal | 产品+用户 | S | 低 | P2 | 未实现 |
| F-25 | 阻塞后清 tty 输入缓冲 | Bug+交互 | 用户+工程 | S | 低 | P2 | 未实现 |
| F-26 | 批量范围默认排除废片 | 需求+交互 | 产品+用户 | S | 低 | P2 | 未实现 |
...
| F-31 | 删门面 find_image_by_path 死代码 | 纯清理 | 工程 | S | 低 | P3 | 未实现 |
| F-32 | 预设播种每调用重算 LUT | 纯清理+性能 | 工程 | S | 低 | P3 | 未实现 |
| F-33 | 迁移残留 ensure_column_dropped x4 | 纯清理 | 工程 | S | 低 | P3 | 未实现 |
