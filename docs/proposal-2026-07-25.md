...
| T-23 | ~~AI 评估失败只报裸数据库 ID，且一次只保留最后一条~~（**已完成**，2026-08-06 复核后两半都属实、照修。行号已漂到 `i18n.cpp:1094-1099` 与 `browse.cpp:1430-1433`，缺陷本身一字未变。**裸 ID** 改成优先报 `file_name`（跟界面其它每一处一致），查不到图片记录时才回落到 ID，不整个吞掉提示 - "图片记录找不到"本身就是一种失败原因。**只留最后一条**：`LastFailure` 换成 `FailureReport{last_image_id, last_error, total_failed}`，`take_last_failure()` 更名 `take_failure_report()`，张数报**累计值**，>1 时进文案（`已有 37 张评估失败，最近: IMG_0042.JPG - 网络请求失败`）。这里的累计 vs 增量是本条的关键：code review 指出，最初那版写成"自上次取走以来的增量"时，网络超时那种一条一条落地的失败（落地间隔远大于 300ms 的 poll 周期）每次都只增了 1，状态行退化回刷屏、一个总数都看不到，正是 U-12 点名的场景没修掉；改累计值后同一条路径上会一路涨到 37。`core/tests/evaluation_worker_test.cpp` 里 "the reported total keeps climbing across takes" 就是这条约束的哨兵。范围上多做一处：`QueueStatus` 加 `failed` 并进 `/tasks` 文案（0 时整句不提失败），状态行是一次性的、错过就没了，`/tasks` 是唯一能事后回看的地方 - 原条目最后一句"`/tasks` 只报排队/处理中数量"正是指这个缺口） | User | `missing-feedback` | P2 | S | `cli/i18n/i18n.cpp:993-999`、`cli/commands/browse.cpp:1172-1175` | ~1h (AI) | none |
