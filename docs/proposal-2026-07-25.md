...
| T-23 | ~~AI 评估失败只报裸数据库 ID，且一次只保留最后一条~~（**已完成**，2026-08-06 复核后两半都属实、照修（行号已漂到 `i18n.cpp:1094-1099` 与 `browse.cpp:1430-1433`，缺陷本身一字未变）：**裸 ID** 改成优先报 `file_name`（跟界面其它每一处一致），查不到图片记录时才回落到 ID - 不整个吞掉提示，因为"图片记录找不到"本身就是一种失败原因。**只留最后一条** 改成累加：`LastFailure` 换成 `FailureReport{last_image_id, last_error, count}`，`take_last_failure()` 更名 `take_failures()`，count 是"上次报过之后又失败了几张"，>1 时文案报数字（`37 张评估失败，最近: IMG_0042.JPG - 网络请求失败`）。原来的覆盖式赋值是"800 张全失败只看到零星几条"的直接成因：后台线程处理请求远快于 300ms 一次的 poll。范围上多做一处：`QueueStatus` 加 `failed` 累计值并进 `/tasks` 文案（0 时整句不提失败），因为状态行是一次性的、错过就没了，`/tasks` 是唯一能事后回看"到底挂了多少张"的地方 - 原条目最后一句"`/tasks` 只报排队/处理中数量"正是指这个缺口） | User | `missing-feedback` | P2 | S | `cli/i18n/i18n.cpp:993-999`、`cli/commands/browse.cpp:1172-1175` | ~1h (AI) | none |
