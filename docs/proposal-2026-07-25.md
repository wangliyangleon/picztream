...
- **T-15 / T-25 → T-16**（原含 T-14，已解除）：两者都要新增或修改"一批图片的作用域 + 排除规则"的使用点。T-16 把 scope 解析与排除策略收进 `core` 并统一 headless 与交互的语义；先做 T-16 可以避免批量套风格与 agent passthrough 各自成为第四、第五份实现。**T-16 不因 T-14 收口而失效**：它记录的核心缺陷是 headless `cmd_dedup`（`commands.cpp:151-205`，不读 `settings.dedup_reject`、不做废片排除）与交互 `/dedup`（`browse.cpp:290-295`，做排除）**同名操作语义分叉**，这与 curate 去留无关。curate 定为 agent 独占后，`agent/stages/curate.py:36` 那份硬编码中文字面量成为该路径的唯一实现，T-25 的优先级相应上升。
