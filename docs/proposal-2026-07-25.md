...
| T-9b | 阻塞期间没有中途取消：机器人能掐掉正在跑的 Dedup，坐在 TUI 前的人不能 | User | `missing-feedback` | P1 | M | `core/dedup/dedup.h`、`core/tournament/tournament.h`（两处均无任何 cancel/stop_token）、`agent/session/worker.py:47` | ~3h (AI)，先与 T-8 一起出设计 | T-8 |
...
依赖成因说明（T-14 解除、T-9 拆分后共 7 条有依赖，其余 26 条为空）：
...
- **T-9b → T-8**：core 侧目前没有任何取消机制（`dedup.h`/`tournament.h` 的 `cancel`/`stop_token` 零命中），要能在比较之间被打断就得动 headless 的原子调用契约——与 T-8 是同一片地，两条应一起设计而不是各改各的。现成的依据是同一能力上的不对称：`worker.py:47` 的 `KILLABLE_STAGES` 含 `Dedup`，机器人能掐掉正在跑的 dedup，坐在 TUI 前的人不能。
...
graph LR
  ...
  T8[T-8 headless 长运行可观测/可取消] --> T9b[T-9b TUI 中途取消]
