...
| T-9b | 阻塞期间没有中途取消：机器人能掐掉正在跑的 Dedup，坐在 TUI 前的人不能 | User | `missing-feedback` | P1 | M | `core/dedup/dedup.h`、`core/tournament/tournament.h`（两处均无任何 cancel/stop_token）、`agent/session/worker.py:47`；PRD 见 `docs/Dedup_Cancel_PRD.md` | ~3h (AI) | ~~T-8~~ none |
