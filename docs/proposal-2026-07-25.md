...
| T-16 | scope 解析与"排除废片"策略在 core/cli/agent 各一份，headless 与交互语义已经分叉（**2026-08-13 复核：本条仍然成立，但原文举的证据已过期、且实际缺陷更严重**。PRD 已立项，开在 GitHub Issues [#23](https://github.com/wangliyangleon/picztream/issues/23)。原文说"headless `cmd_dedup` 不做废片排除、交互 `/dedup` 做排除"，今天两边都排 - 因为 `4cc6549`（2026-07-21，eval 解耦）把排废片挪进了 core 且**无条件**，理由成立（keep 改"留最新"后废片可能当上 keeper 把好邻居打成重复），但它没注意到 cli 层已有一份受 `settings.dedup_reject` 管的条件排除，于是静默压死了那个开关。后果是两处**用户可见但无声**的失效：`dedup_reject=true` 完全无效、`/dedup #废片` 是静默 no-op。这两条在 2026-07-11 随 F-26 真机验证通过过，属**回归**，已坏 22 天；根因是 cli 那个开关零测试覆盖，core 侧改排除策略时没有任何东西会红。排除规则的实现份数也已从原文记的 3 份涨到 6 份，多出 `core/export/export.cpp` 与 `core/curate/curate.cpp`。另发现第三处分叉：headless 的 scope 解析不认系统标签英文别名，`pzt dedup --scope '#Reject'` 报 `tag_not_found` 而 `/dedup #Reject` 可用） | Architect | `module-coupling` | P1 | L | `cli/commands/browse.cpp:231,265,581`、`cli/commands/commands.cpp:148`、`core/tournament/tournament.cpp:123-135`、`core/dedup/dedup.cpp:184`、`core/curate/curate.cpp:224`、`core/export/export.cpp:236-249`、`agent/stages/curate.py:46` | ~1-2 days (AI)，PRD 见 [#23](https://github.com/wangliyangleon/picztream/issues/23) | none |
...
> **2026-08-13 复核**：上一段原本举的证据是"headless `cmd_dedup`（不读 `settings.dedup_reject`、不做废片排除）与交互 `/dedup`（做排除）同名操作语义分叉"，**该证据已过期** - `4cc6549` 把排废片挪进 core 之后两个入口都排。结论不变但依据要换成更强的一条：规则往 core 收只收了一半（dedup/curate 收了，export 没收，cli 那份没删），半吊子状态下 cli 层残留的拷贝被 core 静默覆盖，产生了 `dedup_reject=true` 无效与 `/dedup #废片` 静默 no-op 两处无声失效。详见 T-16 条目与 PRD [#23](https://github.com/wangliyangleon/picztream/issues/23)。
