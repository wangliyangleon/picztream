...
**AG-02 Style 问描述闸门对文本零分类:取消/提问会被当成风格描述**
类别: Bug + 交互打磨 | 来源视角: 用户
...
**AG-04 Deliver 闸门做任何调整都会重新问一遍风格**
类别: Bug + 交互打磨 | 来源视角: 用户
...
**AG-06 交付导出失败被 optional 语义吞掉,收尾却说"这批就处理完啦"**
类别: Bug | 来源视角: 用户 + 架构
...
按预期价值排序:
1. **"不要滤镜/原图直出"路径**:与 AG-01/AG-02 联动,风格闸门支持 skip,StyleApplyAll/Deliver 按无风格走(管线已天然支持 `chosen_recipe=None` 的空跑)。
...
4. **风格候选一览**:Style 闸门支持"有哪些风格?"返回 9 个 preset 名 + 一句话描述(style_matcher 里现成的 `_PRESET_DESCRIPTIONS`);进阶版发多风格对比预览(算力代价大,可后置)。
...
1. **第一波(半天到一天)**:AG-03(协议加字段,防状态污染)→ AG-05(StageStarted 时机)→ AG-06(交付失败误报成功)。三条都是 S 难度、真机每批可见或危害大。
2. **第二波(一到两天)**:AG-01 + AG-02 + AG-16.1 作为一个"Style 闸门健壮化"小增量一起做(同一片代码,分开做会互相返工);顺手 AG-04(闸门重复提问)。
