...
### `core/export/export.cpp` 的跳过原因改成结构化枚举

`ExportSkipped::reason` 原来是 `core` 层硬编码的中文字面量（"源文件缺失"/"解码失败"/"应用风格失败"/"编码失败"），英文模式下 `pzt export` 打印跳过原因会中英文夹杂——`cli/i18n` 够不着 `core` 直接产出的字符串。改成 `enum class SkipReason { SourceMissing, DecodeFailed, RenderFailed, EncodeFailed }`（跟项目里其它结构化错误类型一致），`core/api.h` 重导出，`cli/i18n` 新增 `export_skip_reason(SkipReason)` 做翻译，`cmd_export` 调用点套用。

新增了 `DecodeFailed` 的单元测试（构造一个内容非法但扩展名是 `.jpg` 的"照片"，应用一个真实 recipe 触发烘焙路径）。`RenderFailed`/`EncodeFailed` 没有对应测试——核实过在当前 schema 下不可达：`images.recipe_id` 的外键是 `ON DELETE SET NULL`（`core/db/schema.cpp`），没法人为构造出指向不存在行的 `recipe_id` 来触发 `RenderFailed`；`EncodeFailed` 只在图片尺寸非正时触发，而 `export_tag` 管线里的 `DecodedImage` 全部来自"解码一张真实照片"，尺寸必为正，没有可行的构造路径。这两个分支保留为纯防御性代码。

真机验证过：故意删除一张已打标签图片的源文件，`PZT_LANG=zh`/`PZT_LANG=en` 分别跑 `pzt export`，跳过原因文本正确跟随语言切换。
