...
> **这张表复现不出来，当一次性观测看，别当验收依据。** 2026-08-03 晚同一版提示词
> （2268 字符）、同一批用例重跑，`要活泼点的` 与 `标签叫朋友圈` 都返回了结构不对
> 的东西（`unknown_action ... None`）。本地模型在这个提示词长度上本来就在稳定性
> 边缘，输出不确定。**本票的验收依据是文末那条用户真机复验，不是这张表。**
> 后续也不要再拿本地模型跑这类验证 - 用户 2026-08-03 明确说了本地模型太差，
> 需要验的由他手工在真机上做。
