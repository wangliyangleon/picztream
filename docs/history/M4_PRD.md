# PicZTream (PZT) Milestone 4 产品需求文档（Agent 半自动 · 增量一：粗筛交付最小闭环 + Telegram 接入）
...
5. **Telegram 接入**：官方 Bot API 收原图（按**文件/document** 收，保 EXIF）+ 收意图消息；回进度、回交付的 keeper（文件）。控制与像素同一条通道。
...
- **Telegram 之外的 IM**（WhatsApp 官方/非官方、其它平台）：见文末 TODO；本增量只做 Telegram（watch-folder 作为开发/测试期的等价传输保留）。

## 核心用户故事

出门玩了一天，手机拍了约 40 张。回到住处，把这些照片**按"文件"全发给** Telegram 上那个 PZT bot，再发一句"帮我筛一下，留 9 张"。agent 回一句"好，40 张里帮你筛 9 张，开跑~"，随后回报进度（"评估 24/40…"）。跑完把 **9 张 keeper（原画质文件）+ 一句摘要**发回（"从 40 张里去掉 6 张重复、5 张不达标，给你挑了这 9 张"）。用户看了说"第 3 张换一张""其实留 12 张"，agent 只重挑、很快回新结果。满意就结束。

## 功能需求

### 触发与意图

- **意图消息触发**：照片经 Telegram 到达、无活跃 Run → 归到该用户一个 `Collecting` Run；意图消息（"筛一下留 9 张"）到达即解析、组装 Plan、开跑。照片与意图可任意顺序到。
...
- **`Ingest`**：把 Telegram 收到的原图文件落到暂存目录、`create_project`（复用 core）。关键 Stage（失败则 Run `Failed`）。
...
- **`Deliver`**：复用 `export_images` 导出成品；经 Telegram 回传。**唯一非幂等点**：发前先持久化"本 Run 已交付"标记（或传输层去重），不重发。
...
- 官方 **Bot API**（`python-telegram-bot`）——无商业认证、无计费、无 24 小时窗口/模板、无封号风险；用户先 `/start` 一次，之后 bot 自由收发。
- 传输适配器在统一接口后（另有 watch-folder 实现供开发/测试）。收：意图文本、原图**文件（document）**（保 EXIF）；发：文本、照片预览、文件。
- 控制与像素同一条 Telegram 通道：预览发**照片**（压缩、快、够瞄），原图输入与最终交付发**文件**（字节精确）。默认 bot 收 20MB / 发 50MB，手机 JPEG 够用；RAW/大文件走自搭 local Bot API server（2GB），属后续。
...
- **确定性 & 可测**：驱动器零 LLM、纯确定性；LLM 只在意图/调整解析与 Plan 组装处；三个注入缝（假 Stage / 假子进程 runner / 假传输）让整套编排脱离真 core、真模型、真 Telegram 可单测。
...
- 新增：`core` 侧 `Curate`（确定性策展，经 `pzt curate` 暴露）；一组 headless 子命令；`agent/` 全部（传输 v1 = Telegram Bot API、路由、驱动器、Stage 实现、意图/调整 composer、状态存储）。
- provider 不变（本增量只云端）；`Provider::Local` 属 TODO。

## 验收标准

- **端到端**：Telegram 按文件发 N 张 + 一句意图 → 收到 keeper 文件 + 结果摘要；watch-folder 等价路径同样跑通。
...
- **文件发送的 UX 摩擦**：Telegram 里"按文件发"是发送时的选择（不选就压缩），几十张操作比直接发照片多一步。缓解：预览环节用照片、只有原图输入与最终交付用文件；真机观察摩擦是否可接受。
...
- **其它 IM 接入**：WhatsApp（官方 Business API / 非官方库）及其它平台，作为可插拔传输适配器加进来，编排骨架不变。
