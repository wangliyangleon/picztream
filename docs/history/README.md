...
- `Intent_Curation_PRD.md` + `issues/intent-curation/` + `Research_Selection_And_VLM_Limits.md` - 意图驱动的跨簇选片（2026-08-03 收口，13 张票全部关闭）。**不属于 `docs/proposal-2026-07-25.md` 的 33 条提案，是一次独立立项**。问题是 `pzt curate` 的两步里第二步一直是空白：开 AI 时选择是字面意义的 `std::sample` 随机抽样（W2026-07-21 的 eval 解耦拿掉 `overall_score` 之后，跨簇比较失去了唯一的可比标量，而锦标赛只解决簇内），关 AI 时只按 `captured_at` 散开，用户意图里最有信息量的"用途"那一维流到 Curate 只被当字符串打了个标签。这一刀让选择真正吃意图：描述加 `content` 字段、多样性预筛出预选集（`min(ceil(max(1.5,M)·N), 候选集)`，评估次数与图库大小无关）、curate 内部只评估预选集并由模型按选片简述选出并排序、顺带产出文案，闸门与两级进度在 TUI/headless 两个入口上分别接线。**本增量推翻两处已归档决策**：`W2026-07-21_PRD.md` 的"agent 不再整批跑评估"（现在的评估只跑预选集，不是把删掉的东西加回来）、以及"视觉推理归 core / 语言推理归 agent"那条轴（改述为"关于照片的推理归 core"，见 `docs/adr/0001-core-hosts-photo-reasoning-even-when-text-only.md`）。PRD 顶部的归档说明记了实现期的三处修订与四条风险的结局；逐票拍板与落地记录在 `issues/intent-curation/`（`00-README.md` 是索引），文献调研在 `Research_Selection_And_VLM_Limits.md`
