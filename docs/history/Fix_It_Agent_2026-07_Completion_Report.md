# PZT Agent 层三视角评审 Fix-it 完成报告(评审 2026-07-17,收口 2026-07-19)

> **状态:已收口并归档。** 全部可行动项(P1 6 条 + P2 6 条 + P3 AG-16.2/16.3/17/18/20/21 + AG-16.1/16.4基础版)已实现并真机验证通过,agent 测试 252 → 313 条全绿;AG-14 另含 C++ headless `pzt delete`(build/ 与 build_release/ 均重建)。按设计保留不动:AG-19(观察记录)、AG-16.5(多 chat_id,留部署周)、AG-16 多风格对比预览(后置)。逐条 commit 与修复记录见下文各条目"修复记录"及末尾"收口小结"。
>
> 针对 M4 增量一 + W2026-07-15 AgentRuntime 重构后的 `agent/` 全量代码(基线 commit `14bee68`,252 条测试全绿)的一次专项评审。三个视角:**用户**(模拟 Telegram 端完整工作流与非常规输入)、**架构**(模块与流程设计的漏洞/过度设计/扩展性)、**工程**(代码级 bug/性能/冗余)。评级定义沿用 `docs/history/Fix_It_Night_Review.md`(难度 S/M/L,复杂度 低/中/高,优先级 P0-P3,类别与来源视角同款),编号用 `AG-` 前缀与 CLI/core 的 `F-` 区分。
...
**AG-14 agent 状态目录无限增长,无任何清理/保留策略** ✅ 已修复
...
修复记录(2026-07-18, 拍板 N=7 天 + 同时加 headless delete):
- **C++**: `cli/commands/commands.cpp::cmd_delete` 加 `--json`/`--force` headless 路径(跳过交互 stdin 确认, 项目名=run_id), 复用 emit_json/find_project_by_name/delete_project; `headless_smoke.sh` 加 delete 断言; 重建 build/ 与 build_release/。
- **agent 终态即删**: consumer `_cleanup_run_files` 删该 run 的 incoming/preview/staging + delivered marker(保留 deliver-out+JSON), 在 `_on_run_finished`/取消回执/`_do_cancel` 两支/bootstrap 自愈处调用; `_handle_photo` 每次入 incoming 后删 telegram-inbox 下载源(`_discard_download`)。
- **启动低频清扫**: RunStore `terminal_runs_older_than`/`delete_run`; consumer `_sweep_terminal_runs`(bootstrap 首步)对终态超 7 天的 run 调 `pzt delete --force` + 删 JSON/文件, 容忍 project_not_found。
- 测试: headless_smoke 53 绿 + cli_tests 34 绿; agent 全量 291 条绿(+5)。

---

**AG-15 预览图无编号,"换掉第3张"全凭用户数图** ✅ 已修复
类别: 交互打磨 | 来源视角: 用户
...
修复记录(2026-07-18): transport 链(base/telegram/telegram_client/watchfolder)`send_photo` 加 `caption`(默认 None); worker `_send_preview_media` 加 `numbered` 参数, Deliver 调用传 True 逐张带"第 N 张", 图+文件都发失败时发"第 N 张预览发送失败"占位保序; StyleApplyAll 单张不编号。全量重发只发变化张的优化后置(需 diff 上次 selected)。transport+worker 改动 + 两条测试(caption 透传/失败占位)+ gate-walk caption 断言, 全量 297 条绿。
...
**AG-16 扩展提案打包(用户模拟视角产出,均需产品拍板)** ✅ 可落地项已全部落地
类别: 需求proposal | 来源视角: 用户

状态(2026-07-18/19): 1/2/3/4 均已落地; 5(多 chat_id)按设计留部署周不动; 4 的进阶版"多风格对比预览"算力代价大、后置。
...
**AG-18 视频等不认识的消息形状只在服务端打印,用户侧无反馈** ✅ 已修复
类别: 交互打磨 | 来源视角: 用户
...
修复记录(2026-07-18): `_handle_update` else 分支保留诊断打印, 追加 best-effort 回执"目前只认识照片哦..."(try/except 包住, 发失败不触发外层误导文案)。加一条测试, 全量 310 条绿。
...
**AG-21 常驻进程只有 print,无带时间戳的落盘日志** ✅ 已修复
类别: 工程规范 | 来源视角: 架构
...
修复记录(2026-07-18): 新增 `log_setup.configure_logging(state_dir)`(pzt.agent logger + console StreamHandler + RotatingFileHandler(agent.log, 5MB×3), 带时间戳/级别/logger 名, 幂等); consumer/worker/transport/run_telegram 各模块 `_log = getLogger("pzt.agent.<name>")`, ~30 个 print 一比一换成 `_log.info`, 错误点升 `warning`, worker 崩溃点用 `_log.exception`(取代 traceback.print_exc); run_telegram.main 早调 configure_logging。agent 运行时 print 清零, 加两条 log_setup 测试, 全量 313 条绿。
...
3. **第三波**:P2 里按真机痛感排(建议 AG-09 caption、AG-10 retryable、AG-08 先行,AG-11/AG-12/AG-14 属于"跑得越久越重要"的后台健壮性,部署周(W2026-07-15 目标四)开始前做完最合适)。**✅ 已全部完成(2026-07-18): AG-08 `c80fb65`、AG-09 `8c9afae`、AG-10 `072682f`、AG-11 `08ca3a2`、AG-12 `223df5f`、AG-14 见本轮 commit。剩余 P2 未在本三波点名的 AG-07/AG-13/AG-15 也已补齐(2026-07-18): AG-07 `639824b`、AG-13 `97e3395`、AG-15 见本轮 commit。至此 P2 全清, 仅剩 P3(AG-16 其余提案 / AG-17/18/19/20/21)。**
4. P3 不排期,随手或等触发。**✅ 已全部处理(2026-07-18): AG-16.2/16.3(/命令快路径、进度原地编辑)、AG-17(退避)、AG-18(未知消息回执)、AG-20(小额清理四项)、AG-21(logging 落盘)均已落地; AG-16.1/16.4 已在第二波完成。按设计不动的两项: AG-19(观察记录,等下一个真实新 Stage 再决定是否抽象)、AG-16.5(多 chat_id,Eng Design 明确留部署周、不预留半成品)、AG-16 的"多风格对比预览"(算力代价大,后置)。**

---

## 收口小结(2026-07-18)

整份 fix-it 清单的全部可行动项已完成: **P1 6 条 + P2 6 条 + P3(AG-16.2/16.3/17/18/20/21)**。agent 测试从 252 → 313 条全绿; AG-14 另含 C++ headless `pzt delete`(build/ 与 build_release/ 均重建, headless_smoke 53 + cli_tests 34 绿)。按设计保留不动: AG-19(观察记录)、AG-16.5(部署周)、AG-16 多风格对比预览(后置)。
