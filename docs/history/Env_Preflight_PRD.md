...
> ### 其余验收状态
>
> 其余标准均已核对通过（隔离环境里用 pty 驱动真实 release 二进制、对真实本地 Ollama 的三条路径实跑、真实 `main()` 的启动日志顺序）。两处未能构造：
>
> - (a-2) 的**渲染失败**分支需要写 stdout 失败或 tmux passthrough 关闭，后者在进主循环之前就被既有 fail-fast 拦掉，从外部造不出来。解码失败分支已实测覆盖，两支共用同一套机制。
> - 返工后的**闸门等的是不是真人按键**：插桩证实它是阻塞读并消费一个字节（`isatty=1 read_ret=1`），但 pty 里 `script` 会往伪终端注入一个 EOT 把它满足掉，模拟证不了这一条，留待真机复验。
>
