...
> **归档说明（2026-07-30）**：四段能力已全部实现。**真机验收推翻了本文档的一条目标和一条文案**，见下面"真机验收推翻的两条"。
>
> 实现落在八个提交：
> - `c99d8c8` (b) Telegram 凭证：类型化错误翻成人话 + 退出码 2
> - `a769c0b` (a) A.1：`kitty_support_likely()` 白名单纯函数 + 9 个单测
> - `533f5f5` (a) A.2：文案 + `warn_unsupported_terminal` 开关
> - `7f19b0b` (a) A.3：session notice 通道
> - `fe12672` (a-2) B.1：两条死文案改走 notice 通道，按种类去重
> - `fa6d3ca` (c)/(c-2) D.1：`compose/preflight.py`，一次 `GET /api/tags` 分开两种失败
> - `f851e65` (c)/(c-2) D.2/D.3 + `PZT_AGENT_META_PROVIDER`：接进 `main()`，仅告警不拦启动
> - `4b2cb89` (a) 真机反馈返工：提示改到进备用屏幕之前 + 等一次按键
>
> ### 真机验收推翻的两条
>
> 2026-07-30 在 macOS 自带 Terminal.app 里实跑，结果是：**整屏乱码、全程没有任何提示**，按 `q` 退出之后才看到那段话。两处问题：
>
> 1. **本文档"目标"一节的"提示不阻塞：不需要按任何键确认"作废。** 它写在"banner 一定看得见"这个假设之上，而该假设是错的：banner 画在图片序列之前，不认识 Kitty 协议的终端会把紧随其后的 APC 序列整段当普通文本打出来，几百字节 base64 一换行就把画面顶上去，banner 连同边框一起被滚掉（字节偏移：提示在 1772、图片序列在 1860，只隔 88 字节）。**恰恰在这条提示最该出现的终端里，banner 是最不可能被看见的位置。** 改成进备用屏幕之前打在真实终端上、等一次按键；仍然不阻止进入。
> 2. **文案的"图片区会是空白的"是事实错误。** 空白只是 Ghostty 关掉 passthrough 时的样子，Terminal.app 下是满屏乱码。已改成"可能是乱码或空白"，并加断言锁住。
>
> 教训记在这里：pty 模拟只能验证"字节有没有按预期写出去"，验证不了"终端拿这些字节怎么办"。凡是结论依赖终端行为的，模拟不能替代真机。
>
> ### 验收状态：已全部通过（2026-07-31）
>
> 返工后由用户在真实 Terminal.app 与真实 Telegram bot 上复验，**两侧均通过**：`pzt open` 的闸门确实在进备用屏幕之前出现并等一次按键；agent 侧真 bot 跑一轮无回归。其余标准此前已核对（隔离环境里用 pty 驱动真实 release 二进制、对真实本地 Ollama 的三条路径实跑、真实 `main()` 的启动日志顺序）。
>
> 仅剩一处**未能构造**、按已知边界记档：(a-2) 的**渲染失败**分支需要写 stdout 失败或 tmux passthrough 关闭，后者在进主循环之前就被既有 fail-fast 拦掉，从外部造不出来。解码失败分支已实测覆盖，两支共用同一套机制。
>
> ### 验收期间发现的一处不对称（已单独修掉）
>
> agent 侧模型名可经 `PZT_AGENT_OLLAMA_MODEL` 覆盖，**地址却不能**（硬编码常量），导致"模型没 pull"能靠环境变量轻松构造、"服务连不上"却必须真把 Ollama 停掉，而 brew services 托管的 ollama 被 `pkill` 之后 launchd 会立刻拉回来。已补 `PZT_AGENT_OLLAMA_BASE_URL`（commit `d4b86b2`），这条路径从此有受支持的构造方式。注意这**不解决** PRD 风险 3 记的那件事：agent 与 core 仍是两份独立配置，只是 agent 这一份现在可覆盖了。
>
...
* **(a) `pzt open` 在终端可能不支持 Kitty 图像协议时给出提示**，并指名修复动作（装 Ghostty），但**不阻止进入**
  * ~~提示不阻塞：不需要按任何键确认~~ **（真机验收作废，见顶部归档说明第 1 条；改为进备用屏幕之前等一次按键，但仍不阻止进入）**
...
* 在 iTerm2 或 Terminal.app 里跑 `pzt open`：~~banner 第一帧出现提示~~ **（返工后：进备用屏幕之前在真实终端上出现完整提示 + 等一次按键）**，指名装 Ghostty
* ~~提示不阻塞：不需要按任何键确认，`h`/`l` 等按键行为不变~~ **（返工后作废：按任意键继续；进入之后 `h`/`l` 行为不变）**
