...
> **已归档(2026-07-19)**：本文档覆盖的两刀均已完成——第一刀（9 个 City+Year 内置预设 + 颗粒 pass）、第二刀（`VersionParams` 加对比度/饱和度/黑色/白色四个旋钮）；数值口径统一到 `-100..100`（目标二第 4 项）。**但 PRD 目标二范围第 3 项「几何变换（裁切/水平矫正）」本周未落地、本文档也未覆盖，已顺延**（见 `docs/W2026-07-15_PRD.md` 目标二状态与 `docs/SPEC.md` 未来/搁置一节）。本周开发目标全貌见 PRD。
