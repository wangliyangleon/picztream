...
3. **跨簇挑 N**：
   ...
   - 若簇数 < N：只返回每簇一张代表（覆盖所有场景），**不回填同簇的非代表成员凑数**——`returned = 簇数 < requested`，如实反映，不报错。~~早期设计曾打算"剩余名额从各簇次优图按 overall_score 补齐"，真机验证时发现这会在候选稀缺、彼此近似的场景下把近重复图也凑进最终 N 张，直接违背 curate 存在的多样性目的（例子：只有 2 张互相近重复的候选、`count=2`，回填版本会把两张都选出来）。已改成不回填，宁可 `returned` 少于 `requested`。~~
4. **时间铺开（tie-break 层面）**：跨簇挑选在 `overall_score` 相等时，优先让入选集的 `captured_at` 分布更散（简单实现：相等分数下，选与已入选时间差最大的），让 N 张更像"一天的故事"而非"扎堆一个时段"。
5. **候选/簇数不足 N**：候选总数不够、或簇数不够（第 3 步的"簇数 < N"分支）都不报错，`requested=N, returned=M`（M<N）如实反映——交给 agent/用户知晓。
