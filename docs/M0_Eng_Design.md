...
`g` + `e`(规划中,increment 6.6):在 `g` 前缀下加一条导出快捷方式,复用同一套编号规则选标签(0-9),或者(仅当当前处于 `g`+数字筛选视图时)再按一次 `e` 表示"就导出当前筛选的这个标签",省去在筛选状态下还要再报一次编号。确定标签之后复用 `space c` 已经落地的 `read_text_line` 读入目标文件夹路径(同一套 Esc 取消/退格/UTF-8 拼接逻辑,不新写一套文本输入)。这条快捷方式不做 `--link` 模式切换,固定用 `LinkMode::Copy`——需要软链继续用独立的 `pzt export --link` 命令,菜单里加一层"复制/软链"选择对这个高频操作来说收益不大,不值得多一次按键和多一层状态。`g` + `e` + `e` 在未处于筛选视图时按其它子菜单"无效键静默取消"的一贯风格处理,不报错、不提示。这条不需要改动 `core`——`exporting::export_tag(tag_id, output_folder, link_mode)` 已有的签名就够用,纯粹是 `cli` 层把已有的 `tags_for_menu`/`reject_tag_id`/`active_filter_tag_id`/`read_text_line`/`export_tag` 拼起来。
...
2. **`core` 实现**:量级明显大于其他步骤,实现阶段进一步拆成有序 increment,每个 increment 结束都要能实际跑 `pzt` 命令手动验证,而不是最后一起验收:
   ...
   - **increment 6**:预取/缓存 + 全键盘交互循环 + Kitty 渲染,对应验收标准第 2 条,是 core 实现里最大的一块,内部再拆成有序子步骤,每个子步骤结束都要能实际跑 cli 验证:
     ...
     - **6.6(规划中,未实现)`g e` 导出快捷方式**:在浏览循环里直接导出,不用退出再敲 `pzt export`。设计见上文"浏览与选片(核心交互)"一节 `g` + `e` 的说明,纯 `cli` 层改动,`core::exporting::export_tag` 接口不需要变。落地时预计新增一个类似 `handle_g_export_flow` 的函数(仿照 `handle_create_tag_flow` 两段式:先单字节选标签编号或 `e`,再 `read_text_line` 读路径),在 `cmd_open` 里 `g` 的分支里接入;`pzt tag list` 之外这次也不新增任何调试子命令,直接在交互循环里验证
