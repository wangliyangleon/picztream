...
`g` + `e`(已完成,increment 6.6):在 `g` 前缀下加了一条导出快捷方式,复用同一套编号规则选标签(0-9),或者(仅当当前处于 `g`+数字筛选视图时)再按一次 `e` 表示"就导出当前筛选的这个标签",省去在筛选状态下还要再报一次编号。确定标签之后复用 `space c` 已经落地的 `read_text_line` 读入目标文件夹路径(同一套 Esc 取消/退格/UTF-8 拼接逻辑,没有新写一套文本输入)。这条快捷方式当初固定用 `LinkMode::Copy`,不做模式切换(见下方"导出"一节关于 `--link` 后来被整个移除的说明,这句描述现在已经没有对应的另一个模式可切换了)。`g` + `e` + `e` 在未处于筛选视图时按其它子菜单"无效键静默取消"的一贯风格处理,不报错、不提示。落地时手动验证发现三个真实问题,都已修复,不只是这条快捷方式本身的问题,标准的 `pzt export` 命令同样受影响:(1)`read_text_line` 完全绕过 shell,输入的 `~/xxx` 不会被展开,之前会被字面创建成一个真的叫 `~` 的目录——加了 `expand_home_path` 只处理 `~`/`~/...` 两种形式(不处理 `~user`,M0 单用户场景不需要),`cmd_export`/`handle_g_export_flow` 两个入口统一调用;(2)`export_tag` 内部 `fs::create_directories`/`fs::copy_file`/`fs::create_symlink` 都是会抛异常的重载,目标路径不可写(权限不足、路径某一段已经是个普通文件)时之前会直接抛出 `filesystem_error` 让整个交互循环崩溃——现在包了一层 try/catch,转成新增的 `ExportTagError::IoError`;(3)导出 0 张图片时之前跟正常成功用同一句"已导出 0 张",容易被当成误操作——现在单独判一句"标签下没有图片,未导出"。顺带给 `ExportResult` 加了 `created_output_folder` 字段(`export_tag` 内部导出前先查一次目标是否已存在),导出成功的提示里现在会区分"目录不存在,已新建"和目标本来就有,不然用户分不清是不是打错了路径。
...
- `core/export/`:导出操作(复制/软链、有序编号命名、冲突消歧、缺失源文件容错)——软链这部分 M2 阶段已移除,现在只有复制,见"导出"一节的后续状态说明
...
2. **`core` 实现**:量级明显大于其他步骤,实现阶段进一步拆成有序 increment,每个 increment 结束都要能实际跑 `pzt` 命令手动验证,而不是最后一起验收:
   ...
   - **increment 6**:预取/缓存 + 全键盘交互循环 + Kitty 渲染,对应验收标准第 2 条,是 core 实现里最大的一块,内部再拆成有序子步骤,每个子步骤结束都要能实际跑 cli 验证:
     ...
     - **6.4(已完成) 全键盘交互循环**(`cli`):把预取缓存 + Kitty 渲染器 + 已有的 core 浏览/标签/导出接口接到一起,`pzt open` 从桩实现变成真正的每日可用界面,内部按功能拆成 6.4.1-6.4.7 一串子步骤(三面板布局、标签的 `space` 菜单增删建删、废片系统标签固定占 `0` 号位、`g`+数字筛选视图、最后 6.4.7 收尾)。**6.4.7(已完成)**:退休了当时为单独验证各模块而加的全部临时调试命令——`pzt decode`/`pzt render`/`pzt prefetch`/`pzt browse *`/`pzt tag create|add|remove|replace`(保留 `pzt tag list`),连带清理了只被这些命令使用的死代码(`find_ref`/`print_nav_result`/`print_cap_exceeded`/`resolve_image`);`cmd_open` 退出时新增一行 `key-to-render` 汇总(n/avg/p95/max),不挂在 `--debug` 后面,PRD"简单的延迟日志验证无可感知卡顿"这条验收标准原本只能靠盯着 debug 面板最后 8 行的实时小窗口,现在有了事后能看的汇总;这一步过程中顺带修了一个真实存在、跟这次改动无关的既有 bug——`DebugLogRedirect`/`PrefetchCache` 原来声明在 `cmd_open` 函数的最外层作用域,只有整个函数返回时才析构,导致任何在 `AltScreen`/`CbreakMode` 那个内层块结束之后、`return` 之前打的 stderr 消息(包括这次新加的汇总,也包括一直存在的"已退出浏览"退出提示)实际上永远到不了真实终端——已经把这两个的声明挪进内层块,让它们跟 `AltScreen`/`CbreakMode` 同时析构、及时把 stderr 换回真实终端。
     - **6.5(视时间/需要而定的收尾项,不阻塞前四步跑通)**:降采样兜底阈值调优、`t=s` 共享内存传输介质验证——这两个都在"待确认问题"里放了很久,真机验证数据积累后再定
     - **6.6(已完成)`g e` 导出快捷方式**:在浏览循环里直接导出,不用退出再敲 `pzt export`。设计见上文"浏览与选片(核心交互)"一节 `g` + `e` 的说明,新增 `handle_g_export_flow`(仿照 `handle_create_tag_flow` 两段式:先单字节选标签编号或 `e`,再 `read_text_line` 读路径),在 `cmd_open` 里 `g` 的分支里接入;没有新增任何调试子命令,直接在交互循环里验证。手动验证过程中发现并修复了三个真实 bug(`~` 路径展开、导出目标不可写时的崩溃、0 张导出的误导性提示,详见上文 `g` + `e` 设计段落),都属于导出模块本身的问题,标准的 `pzt export` 命令一并受益
...
- **tmux 切走窗口时 Kitty 图片残留,已确认是 tmux passthrough 的硬限制,M0 阶段不解决**(6.4 阶段真机测试发现):在 tmux 里打开 `pzt open` 后切到另一个 tmux 窗口,图片会一直挡在那个窗口里。根因跟"退出时清一次 placement"是同一类问题的另一种触发方式:Kitty 图片是叠加在文字网格之上的独立合成层,tmux 切窗口只重绘新窗口的文字内容,不会清掉旧窗口留下的图片,而我们的进程还在跑、只是不知道自己被切走了。尝试过两版检测机制:第一版用 tmux 的 `pane-focus-out`/`pane-focus-in` hook,真机测试没用,查证是因为这两个 hook 依赖真实终端级别的焦点事件,同一个 Ghostty 窗口内切 tmux 窗口根本不产生这种事件;`after-next-window`/`after-previous-window` 这类命令 hook 在实测的 tmux 3.7 上也不是有效名字。第二版改用轮询(后台线程每 300ms 查一次 `tmux display-message -p -t <pane> '#{window_active}'`,状态变化时给自己发信号打断阻塞的 `read()`),这一版检测机制本身完全正确——加了临时诊断日志后确认了完整链路(轮询发现变化→发信号→`read()` 收到 `EINTR`→调用 `clear_placement`)每一步都按预期执行。但真机验证图片依然没有消失,进一步直接测试证实了根本原因:**tmux 不会把非当前活跃 pane 发出的 passthrough 内容转发给客户端的真实终端**,不管是谁发的、发的是什么。也就是说,只要我们的 pane 已经不是当前显示的那个,我们发出的清除命令物理上就到不了 Ghostty,等我们重新变成活跃 pane 时再清已经晚了——图片在别的窗口里挡着的这段时间是没法从我们这一侧主动补救的。已确认这是 M0 阶段接受的已知限制,不继续投入(比如通过 `display-popup` 等其它机制间接触发,需要更多研究,超出这次的范围);当前的应对方式是切走前先按 `q` 退出。
...
- （已修复)banner 文案被截断,丢了 "q 退出" 提示(6.4.3 阶段自动化验证发现):`cli/main.cpp` 里 `truncate_text`/`pad_to` 一直是按字节数近似显示宽度(6.4.2 就有的简化,当时只影响信息栏里可能截断的文件名/标签名,截了就截了,不算严重)。6.4.3 把 banner 文案加长("h/l ... j/k ... q 退出")之后,这个近似第一次产生真实后果:UTF-8 里中文字符是 3 字节但终端里只占 2 列,按字节数算出来的"宽度"比实际显示宽度大,导致 70 列宽的终端下banner 文案在真正要用完这两列宽度之前就被判定"超长"截断,末尾的 "q 退出" 直接被切掉。已修复:改成真正解析 UTF-8 码点、按 East-Asian-Width 意义上的"宽字符占 2 列"来计算显示宽度和截断位置(`is_wide_codepoint`/`decode_utf8_at`/`display_width`),不再是字节数近似。这也顺带解决了信息栏文件名/标签名的同类简化。
- （已定位方向,未完全查清)`pzt open` 里连续快速按 h/l 导航(比如 `l,l,h` 三次紧挨着按)之后再按 `q`,在自动化验证里观测到一次真正的长时间无响应(30+ 秒,不是"卡顿"级别),不是之前那次已修复的"worker 不检查 stop 就继续拿新任务"那个问题(用 `sample` 抓栈确认这次不是卡在 `~PrefetchCache()`,而是主线程在等 `get()` 返回、后台 worker 线程仍在 `decode_jpeg_file` 内部做真实的内存分配/拷贝)。初步判断是:连续几次导航,每次 `set_current` 都会让预取窗口纳入新的大图(项目里的真实素材有不少 12MP+),单一 worker 线程要串行解码完积压的好几张大图,每张在 ASan 插桩下的分配/拷贝开销被放大很多,叠加起来可能撑到几十秒——但这只是基于一次 profile 的推测,没有系统性复现或量化,不确定是否还有别的因素。跟"切图卡顿"是同一类"Debug+ASan 构建下真实观感有多少被插桩放大"问题,放一起记录:下次要继续查,应该先在 RelWithDebInfo 构建下重复同样的按键序列,如果同样的操作在没有 ASan 时表现正常,基本可以确认是插桩开销而不是真实死锁;如果 RelWithDebInfo 下依然长时间无响应,那就要认真当成一个并发 bug 去查,不能只归咎于 ASan
...
- 导出目标文件夹如果设在项目 `root_path` 内部(比如项目文件夹下建个 `exported/` 子目录),下次 `pzt rescan` 会把导出产物(复制体)当成磁盘上新出现的照片扫进 `images` 表——`rescan` 是对 `root_path` 做全量递归扫描,导出文件的路径/文件名跟已知源文件不一样,会被当成合法新照片补录进来,污染项目、让这些没打过标签的衍生重复图出现在 `next_untagged` 里。increment 5 没有处理这个问题,只在这里记录;目前的应对方式是提醒用户导出目标文件夹设在项目文件夹之外,以后要不要在代码层面处理(比如禁止 `output_folder` 是 `root_path` 的子路径、或者 rescan 时排除已知导出目录)留待后续再定。（**M2 阶段追加**:原文这里还提到"`--link` 模式下还会出现两条 `images` 记录指向同一份物理文件"——`--link` 已经在 M2 阶段整个移除，这一半问题不会再发生；上面说的"复制体被误扫描"这一半跟 link 模式无关，依然存在，留待后续。）
