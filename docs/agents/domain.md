...
**其一，`docs/SPEC.md` 是每个 session 唯一必读的 ground truth**，先于本文件列的任何东西读。它承载项目定位、模块划分、设计哲学、对外接口轮廓、技术契约。`CONTEXT.md` 只是术语表，**不重复 SPEC 的内容**，也不记实现决策。

**其二，ADR 永远留在仓库、不进 issue tracker**，即便 PRD 与票已经迁到 GitHub Issues。理由与迁移政策见 `docs/agents/issue-tracker.md`。
