...
* 其余非 history 文档：先识别当前 session 的开发意图，再按需读取对应文档，不要预先全读：
  * 开发本周 feature → 读 `docs/W{当前周日期}_PRD.md` 及对应的 `docs/W{当前周日期}_Eng_Design.md`（本周为 `docs/W2026-07-15_*`，可能拆成多份按子系统的 Eng Design）
  * 做 fix-it / 评审修复 → 读 `docs/Fix_It_Night_Review.md`（活跃的全面评审/修复清单，仍有 P3 遗留项）
