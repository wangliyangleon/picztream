...
* 其余非 history 文档：先识别当前 session 的开发意图，再按需读取对应文档，不要预先全读：
  * 开发本周 feature → 读 `docs/W{当前周日期}_PRD.md`（本周为 `docs/W2026-07-15_PRD.md`，仍活跃因目标四部署未完成）；本周对应的四份子系统 Eng Design 已随目标一/二/三/五完成归档进 `docs/history/W2026-07-15_*_Eng_Design.md`，需倒查实现细节时去 history 读
...
M0（MVP）、M1（风格化）、M2（RAW）、M3 的两个增量（选片辅助评分、近似重复检测）、M4 增量一（Telegram 选片-交付闭环，含 agent Style）均已完成。本周 `W2026-07-15` 五个开发目标（含 2026-07-17 追加的目标五 agent 运行时双线程重构）中，本地模型（Ollama）、recipe/滤镜扩展、agent Style、agent 运行时重构已完成；**目标二的几何变换（裁切/水平矫正）未做、顺延**；目标四部署与分发尚未开始。完整现状与路线见 `docs/SPEC.md` 的"现状与路线"一节，本周细节见 `docs/W2026-07-15_PRD.md`。
