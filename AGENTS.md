...
* 其余非 history 文档：先识别当前 session 的开发意图，再按需读取对应文档，不要预先全读：
  * 开发本周 feature → 读 `docs/W{当前周日期}_PRD.md` 及对应 Eng Design。**当前无活跃周目标**：上一周 `W2026-07-15` 已全部收口，PRD 与五份/四份子系统 Eng Design 均归档在 `docs/history/W2026-07-15_*`（需倒查实现细节时去 history 读）。新一周目标定稿后在 `docs/` 新建 `W{日期}_PRD.md`
