...
* 其余非 history 文档：先识别当前 session 的开发意图，再按需读取对应文档，不要预先全读：
  * 开发本周 feature → 读 `docs/W{当前周日期}_PRD.md` 及对应 Eng Design。**当前活跃周目标 `W2026-07-21`**（PRD 定稿、Eng Design 待产）：eval 解耦重构（文字+硬伤 flag）+ 涉及比较的选择改 AI 锦标赛（pairwise 在 core、bracket 在 agent）+ 全局 AI 开关，分目标一（地基）/目标二（接线）两阶段，见 `docs/W2026-07-21_PRD.md`。上一周 `W2026-07-15` 已全部收口，归档在 `docs/history/W2026-07-15_*`（需倒查实现细节时去 history 读）
...
M0（MVP）、M1（风格化）、M2（RAW）、M3 的两个增量（选片辅助评分、近似重复检测）、M4 增量一（Telegram 选片-交付闭环，含 agent Style）均已完成。`W2026-07-15` 周目标（含 2026-07-17 追加的目标五 agent 运行时双线程重构）已全部收口并归档：本地模型（Ollama）、recipe/滤镜扩展、agent Style、agent 运行时重构、部署与分发（Homebrew tap 分发 `pzt`/`pzt-agent` + README + 静态主页 + 一键 release 自动化）均完成；**目标二的几何变换（裁切/水平矫正）顺延**，已进 `docs/Task_Pool.md`。当前活跃周目标 `W2026-07-21`（PRD 定稿、Eng Design 待产）：eval 解耦（文字+硬伤 flag、不再产跨图分数）+ dedup/curate 涉及比较的选择改 AI 锦标赛（pairwise 在 core、bracket 在 agent）+ 全局 AI 开关，分地基/接线两阶段，见 `docs/W2026-07-21_PRD.md`。完整现状与路线见 `docs/SPEC.md` 的"现状与路线"一节。
