...
M0（MVP）、M1（风格化）、M2（RAW）、M3 的两个增量（选片辅助评分、近似重复检测）、M4 增量一（Telegram 选片-交付闭环，含 agent Style）均已完成。`W2026-07-15` 周目标（含 2026-07-17 追加的目标五 agent 运行时双线程重构）已全部收口并归档：本地模型（Ollama）、recipe/滤镜扩展、agent Style、agent 运行时重构、部署与分发（Homebrew tap 分发 `pzt`/`pzt-agent` + README + 静态主页 + 一键 release 自动化）均完成；**目标二的几何变换（裁切/水平矫正）顺延**，已进 `docs/Task_Pool.md`。`W2026-07-21` 周目标同样已全部收口并归档：eval 解耦（文字描述+硬伤 flag，不再产跨图分数）+ dedup/curate 涉及比较的选择改 AI 锦标赛（pairwise 在 core、bracket 在 agent）+ 全局 AI 开关 + dedup/curate 流程按意图可选化（不再固定全跑）均完成；真机反馈后又追加了一次超出该周 PRD 范围的架构调整（选片确认闸门挪到套滤镜之前，交付不再挂闸门）。手动选片模式明确移出范围，未立项。当前没有活跃周目标。完整现状与路线见 `docs/SPEC.md` 的"现状与路线"一节。
