...
def test_step_returns_false_on_empty_queue(tmp_path):
    ...
    assert env.worker.step() is False
...
def test_classify_collecting_emits_done_with_result(tmp_path):
    ...
    env.jobs.put(ClassifyJob(generation=3, kind="collecting", text="收到几张了",
                              context={"photo_count": 2}))

    assert env.worker.step() is True
...
def test_classify_adjustment_error_maps_to_not_retryable(tmp_path):
    ...
    env.jobs.put(ClassifyJob(generation=1, kind="collecting", text="???",
...
def test_classify_llm_error_maps_to_retryable(tmp_path):
    ...
    env.jobs.put(ClassifyJob(generation=1, kind="collecting", text="筛一下",
...
def test_classify_gate_reply_loads_run_from_store(tmp_path):
    ...
    env.jobs.put(ClassifyJob(generation=1, kind="gate_reply", text="挺好的",
...
def test_classify_refine_plan_passes_context(tmp_path):
    ...
    env.jobs.put(ClassifyJob(generation=1, kind="refine_plan", text="改成6张",
...
def test_compose_success_emits_validated_plan(tmp_path):
    env = make_worker(tmp_path)
    env.jobs.put(ComposeJob(generation=2, intent_text="筛一下留2张"))
...
def test_compose_llm_failure_emits_compose_failed(tmp_path):
    ...
    env.jobs.put(ComposeJob(generation=2, intent_text="???"))
...
def test_drive_start_runs_to_style_gate(tmp_path):
    ...
    env.jobs.put(DriveJob(generation=1, action="start", run_id=run.run_id))
...
def test_full_gate_walk_style_then_apply_all_then_deliver(tmp_path):
    ...
    env.jobs.put(DriveJob(generation=1, action="rerun_style", run_id=run.run_id,
    ...
    env.jobs.put(DriveJob(generation=1, action="resolve_gate", run_id=run.run_id))
    env.worker.step()
    events = env.drain_events()
    gate = events[-1]
    assert isinstance(gate, GateReached)
...
def test_unexpected_exception_emits_job_crashed(tmp_path):
    ...
    env.jobs.put(ClassifyJob(generation=7, kind="collecting", text="x",
                              context={"photo_count": 0}))
