...
def test_crash_after_stage_done_resumes_without_rerunning_it(tmp_path):
    ...
    driver_a = Driver(stages={"Ingest": ingest, "Evaluate": evaluate}, store=store_a)
    ...
    driver_b = Driver(stages={"Ingest": ingest_after_restart, "Evaluate": evaluate}, store=store_b)
    reloaded = store_b.load("run-1")

    driver_b.advance(reloaded)

    assert len(ingest_after_restart.calls) == 0
    assert reloaded.stage_states["Evaluate"] == StageStatus.DONE
...
def test_list_active_finds_run_stuck_mid_run_after_restart(tmp_path):
    ...
    Driver(stages={"Ingest": ingest, "Evaluate": evaluate}, store=store).advance(run)
