#!/usr/bin/env python3
"""Telegram 长驻会话 runner 2.0（W2026-07-15 目标五）：consumer/worker 双
线程运行时，替换 run_telegram.py 的单线程同步主循环——那个形状下
handle_message 内嵌整条 drive 和同步 LLM 调用，处理期间所有入站消息堵
死。2.0 的 consumer（主线程）只做秒级消息反应/事件应用/timers，长活全部
经 job 队列交给 worker 线程。见 docs/W2026-07-15_AgentRuntime_Eng_Design.md。

并行落地期间与旧入口共存；同一个 bot 同时只能跑一个入口（Telegram
getUpdates 单消费者，双开会 409 Conflict）。真机验证通过后本文件转正为
run_telegram.py、删除旧 router。
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Telegram 长驻会话 runner 2.0（consumer/worker 双线程）")
    ...
    try:
        while True:
            try:
                ...
                # 的都在各自边界落过了，这轮当作跳过。
                print(f"[run_telegram2] consumer step 出错，已跳过：{e!r}")
